...
class TenantAwareCollection:
    ...
    async def insert_one(self, document, *args, **kwargs):
        if self._enabled():
            ...
            company_id = authenticated_company_id()
            if document.get(COMPANY_FIELD) is not None and str(document[COMPANY_FIELD]) != company_id:
                raise HTTPException(status_code=403, detail="Cross-company access is not permitted")
            document[COMPANY_FIELD] = company_id
    ...
    async def insert_many(self, documents, *args, **kwargs):
        if self._enabled():
            ...
            for document in documents:
                item=dict(document)
                if item.get(COMPANY_FIELD) is not None and str(item[COMPANY_FIELD]) != company_id:
                    raise HTTPException(status_code=403, detail="Cross-company access is not permitted")
                item[COMPANY_FIELD]=company_id
                scoped.append(item)
            documents=scoped
        return await self._collection.insert_many(documents, *args, **kwargs)

    async def update_one(self, query, update, *args, **kwargs):
        return await self._collection.update_one(_scope_query(query) if self._enabled() else query, _scope_update(update) if self._enabled() else update, *args, **kwargs)

    async def update_many(self, query, update, *args, **kwargs):
        return await self._collection.update_many(_scope_query(query) if self._enabled() else query, _scope_update(update) if self._enabled() else update, *args, **kwargs)

    async def replace_one(self, query, replacement, *args, **kwargs):
        if self._enabled():
            replacement=dict(replacement); company_id=authenticated_company_id()
            if replacement.get(COMPANY_FIELD) is not None and str(replacement[COMPANY_FIELD]) != company_id:
                raise HTTPException(status_code=403, detail="Cross-company access is not permitted")
            replacement[COMPANY_FIELD]=company_id
        return await self._collection.replace_one(_scope_query(query) if self._enabled() else query, replacement, *args, **kwargs)
...
class TenantAwareDatabase:
    ...
    def __getitem__(self, name):
        ...
        return TenantAwareCollection(collection,name)
