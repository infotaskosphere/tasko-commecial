...
function readState() { try { return JSON.parse(fs.readFileSync(LICENSE_ACTIVATION_PATH, 'utf8')); } catch { return null; } }
function writeState(state) { ensureDir(); const tmp = `${LICENSE_ACTIVATION_PATH}.tmp`; fs.writeFileSync(tmp, JSON.stringify(state, null, 2), 'utf8'); fs.renameSync(tmp, LICENSE_ACTIVATION_PATH); }

let cached = readState();
let checking = null;

async function licensingPost(endpoint, body) {
  const response = await fetch(`${LICENSE_API_URL}${endpoint}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: AbortSignal.timeout(10000) });
  let payload = {};
...
async function refreshLicense(force = false) {
  ...
  checking = (async () => {
    try {
      const result = await licensingPost('/licensing/heartbeat', { license_key: cached.license_key, installation_id: cached.installation_id });
      ...
      if (cached.status?.valid && OFFLINE_GRACE_MINUTES > 0 && age <= OFFLINE_GRACE_MINUTES * 60000) return { ...cached.status, reason: 'Licensing server temporarily unavailable; offline grace period active.' };
...
function createEnforcementMiddleware() {
  return async function licenseMiddleware(req, res, next) {
    ...
    if (exempt(pathname) || !LICENSE_ENFORCEMENT) return next();
    const module = moduleForPath(pathname);
    if (!module) return next();
    const status = await refreshLicense(false);
    if (!status.valid) return res.status(403).json({ error: 'LICENSE_REQUIRED', detail: status.reason || 'A valid Taskosphere license is required.' });
    const modules = Array.isArray(status.license?.modules) ? status.license.modules : [];
    if (!modules.includes(module)) return res.status(403).json({ error: 'MODULE_NOT_LICENSED', module, detail: `Your Taskosphere license does not include the ${module} module.` });
    return next();
  };
}
...
async function handleLocalLicenseRoute(req, res) {
  ...
  if (pathname === '/licensing/installation-activate' && req.method === 'POST') {
    try {
      ...
      const now = new Date().toISOString();
      cached = { license_key: licenseKey, installation_id: installationId, installation_name: installationName, activated_at: now, checked_at: now, status: { valid: true, license: result.license, checked_at: now } };
      writeState(cached);
      return res.json(result);
    } catch (error) { return res.status(400).json({ detail: error.message || 'Activation failed.' }); }
  }
