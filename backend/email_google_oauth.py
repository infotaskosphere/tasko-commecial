...
# This router is included inside backend.email_integration.router, which already
# has the /email prefix. Therefore this prefix deliberately starts at /oauth.
router = APIRouter(prefix="/oauth/google", tags=["email-google-oauth"])
...
def _frontend_origin(request: Request) -> str:
    return (os.getenv("FRONTEND_URL") or request.headers.get("origin") or "").strip().rstrip("/")
...
def _safe_return_url(raw: Optional[str], request: Request) -> str:
    ...
    try:
        ...
        if f"{parsed.scheme}://{parsed.netloc}".rstrip("/") == f"{allowed.scheme}://{allowed.netloc}".rstrip("/") and parsed.scheme in {"http", "https"}:
            return candidate
    except Exception:
        pass
    return f"{origin}/settings/email"
...
def _flow(redirect_uri: str, state: Optional[str] = None) -> Flow:
    flow = Flow.from_client_config(_google_client_config(), scopes=[GOOGLE_GMAIL_SCOPE], state=state)
    flow.redirect_uri = redirect_uri
    return flow


async def _save_state(*, state: str, current_user: Any, redirect_uri: str, return_url: str, linked_page: str, auto_sync: bool, label: Optional[str]) -> None:
...
@router.get("/start")
async def start_google_gmail_oauth(request: Request, linked_page: str = Query("all"), auto_sync: bool = Query(False), label: Optional[str] = Query(None), return_url: Optional[str] = Query(None), current_user=Depends(check_module_permission("email_accounts", "create"))):
    redirect_uri = _redirect_uri(request)
    return_url = _safe_return_url(return_url, request)
    state = secrets.token_urlsafe(32)
    flow = _flow(redirect_uri, state=state)
    authorization_url, _ = flow.authorization_url(access_type="offline", include_granted_scopes="true", prompt="consent select_account", state=state)
    await _save_state(state=state, current_user=current_user, redirect_uri=redirect_uri, return_url=return_url, linked_page=linked_page, auto_sync=auto_sync, label=label)
...
@router.get("/callback")
async def google_gmail_oauth_callback(request: Request, code: Optional[str] = Query(None), state: Optional[str] = Query(None), error: Optional[str] = Query(None)):
    ...
    try:
        ...
        if not refresh_token:
            raise HTTPException(status_code=400, detail="Google did not return a refresh token. Please approve offline Gmail access and reconnect.")
        ...
        await db[_email.COL_CONNECTIONS].update_one({"user_id": user_id, "email_address": email_address}, {"$set": connection_doc}, upsert=True)
...
def _credentials_from_marker(marker: str) -> Credentials:
    ...
    credentials = Credentials(token=None, refresh_token=refresh_token, token_uri="https://oauth2.googleapis.com/token", client_id=(os.getenv("GOOGLE_CLIENT_ID") or "").strip(), client_secret=(os.getenv("GOOGLE_CLIENT_SECRET") or "").strip(), scopes=[GOOGLE_GMAIL_SCOPE])
...
def _gmail_record(raw_message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "subject": _email._clean_text(_email._decode_header_str(message.get("Subject", "")), 200),
        "from_addr": from_raw,
...
def _scan_gmail_sync(host: str, port: int, email_addr: str, password: str, max_msgs: int = 50, sender_whitelist: Optional[List[str]] = None, since_date: Optional[str] = None, keywords: Optional[List[str]] = None, keyword_match_mode: str = "or", keyword_case_sensitive: bool = False) -> List[Dict[str, Any]]:
    service = _gmail_service_from_password(password)
    refs = service.users().messages().list(userId="me", q=_gmail_query(since_date, keywords, keyword_match_mode), maxResults=min(max(max_msgs or 50, 1), 500), includeSpamTrash=False).execute().get("messages") or []
    clean_keywords = [str(k).strip() for k in (keywords or []) if str(k).strip()]
    output: List[Dict[str, Any]] = []
    for ref in refs[:max_msgs or 50]:
        try:
            record = _gmail_record(service.users().messages().get(userId="me", id=ref["id"], format="raw").execute())
            subject = record["subject"] or ""
            if clean_keywords:
                haystack = subject if keyword_case_sensitive else subject.lower()
                matched = [kw for kw in clean_keywords if (kw if keyword_case_sensitive else kw.lower()) in haystack]
...
def _test_gmail_sync(host: str, port: int, email_addr: str, password: str):
    try:
        service = _gmail_service_from_password(password)
        actual = str(service.users().getProfile(userId="me").execute().get("emailAddress") or "").strip().lower()
...
# email_integration already has the /email prefix. Mounting this router here
# exposes exactly /api/email/oauth/google/start and /callback.
