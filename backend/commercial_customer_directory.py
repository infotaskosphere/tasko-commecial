...
def _require_platform_owner(current_user: User) -> None:
    if not is_platform_owner(current_user): raise HTTPException(status_code=403, detail="Commercial customer administration is restricted to the platform owner.")
def _public(value: Dict[str, Any]) -> Dict[str, Any]:
    result = dict(value or {}); result.pop("_id", None); return result
def _feature_flags(module_id: str) -> List[str]:
    return [str(page.get("flag")) for page in MODULE_HIERARCHY.get(module_id, {}).get("pages", []) if page.get("flag")]
...
async def update_commercial_license(license_id: str, payload: Dict[str, Any], current_user: User = Depends(get_current_user)):
    _require_platform_owner(current_user); license_doc = await db.commercial_licenses.find_one({"id": str(license_id)}, {"_id": 0})
    if not license_doc: raise HTTPException(status_code=404, detail="License was not found.")
    customer_id = str(license_doc.get("customer_id") or ""); customer = await db.commercial_license_customers.find_one({"id": customer_id}, {"_id": 0})
    if not customer: raise HTTPException(status_code=404, detail="Commercial customer was not found.")
    raw_modules = payload.get("modules", license_doc.get("modules") or []); modules = []
    for raw in raw_modules if isinstance(raw_modules, list) else []:
        module_id = str(raw).strip().lower()
        if module_id in MODULE_IDS and module_id not in modules: modules.append(module_id)
    if not modules: raise HTTPException(status_code=400, detail="Select at least one licensed module.")
    raw_features = payload.get("selected_features", license_doc.get("selected_features") or {}); selected_features: Dict[str, List[str]] = {}
    if not isinstance(raw_features, dict): raise HTTPException(status_code=400, detail="Feature access must be an object.")
    for module_id in modules:
        allowed = set(_feature_flags(module_id)); raw = raw_features.get(module_id)
        if raw is None: selected = sorted(allowed)
        elif isinstance(raw, list): selected = []
        else: raise HTTPException(status_code=400, detail=f"Features for {module_id} must be a list.")
        if isinstance(raw, list):
            for flag in raw:
                flag = str(flag).strip()
                if flag in allowed and flag not in selected: selected.append(flag)
    ...
    await db.commercial_license_customers.update_one({"id": customer_id}, {"$set": {"licensed_modules": modules, "selected_features": selected_features, "updated_at": stamp}})
    await db.companies.update_many({"commercial_customer_id": customer_id, "source": "commercial-license"}, {"$set": {"licensed_modules": modules, "selected_features": selected_features, "license_id": str(license_id)}})
...
import backend.commercial_licensee_stats  # noqa: F401
