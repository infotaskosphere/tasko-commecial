...
def require_document_type(value: Any) -> str:
    ...
    if document_type == "UNCLASSIFIED":
        raise AccountingControlError("Transaction type could not be determined with sufficient evidence; posting requires human review.")
...
def validate_balanced_lines(lines: Iterable[dict[str, Any]], tolerance: Decimal = Decimal("0.00")) -> tuple[Decimal, Decimal]:
    ...
    if abs(total_debit - total_credit) > tolerance:
        raise AccountingControlError(f"Journal entry does not balance: debit {total_debit} != credit {total_credit}.")
...
def _authorized_company(requested_company_id: str, user: User) -> str:
    """Enforce tenant scope; only platform admins may inspect another company."""
    requested = str(requested_company_id or "").strip()
    own = str(getattr(user, "company_id", None) or "").strip()
    if user.role == "admin":
        if not requested:
            raise HTTPException(400, "company_id is required for accounting administration.")
        return requested
    if not own or not requested or requested != own:
        raise HTTPException(403, "Accounting data is restricted to your company.")
    return own
...
async def _append_audit(company_id: str, event_type: str, actor_id: Optional[str], payload: dict, document_id: Optional[str] = None) -> dict:
    ...
    seq_doc = await db.accounting_audit_sequences.find_one_and_update(
        {"company_id": str(company_id or "")}, {"$inc": {"sequence": 1}, "$set": {"updated_at": now}},
        upsert=True, return_document=ReturnDocument.AFTER,
    )
    sequence = int(seq_doc.get("sequence", 1))
    previous_hash = "GENESIS"
    if sequence > 1:
        previous = await db.accounting_audit.find_one({"company_id": str(company_id or ""), "sequence": sequence - 1}, {"_id": 0, "event_hash": 1})
    ...
    event = {"id": str(uuid.uuid4()), "company_id": str(company_id or ""), "sequence": sequence, "event_type": event_type,
             "actor_id": actor_id, "document_id": document_id, "payload": payload, "created_at": now, "previous_hash": previous_hash}
    ...
    async def wrapped(company_id, entry_date, narration, lines, source, source_id, created_by):
        ...
        if result is None:
            ...
            await db.accounting_posting_failures.update_one(
                ...
                {"$set": {"lines": lines, "entry_date": entry_date, "narration": narration, "status": "PENDING_APPROVAL", "approval_required": True, "updated_at": now},
...
async def approve_posting_failure(failure_id: str, body: ApprovalDecision, current_user: User = Depends(get_current_user)):
    ...
    now = datetime.now(timezone.utc).isoformat()
    await db.accounting_posting_failures.update_one({"id": failure_id, "company_id": company_id}, {"$set": {"status": "APPROVED", "approved_by": current_user.id, "approved_at": now, "approval_reason": body.reason.strip()}})
    ...
    try:
        result = await _accounting_core.post_journal_entry(company_id, failure.get("entry_date"), failure.get("narration", ""), lines, failure.get("source", "manual"), failure.get("source_id"), current_user.id)
    except Exception as exc:
        await db.accounting_posting_failures.update_one({"id": failure_id, "company_id": company_id}, {"$set": {"status": "POSTING_FAILED", "last_retry_error": str(exc), "updated_at": now}})
        await _append_audit(company_id, "POSTING_RETRY_FAILED", current_user.id, {"failure_id": failure_id, "error": str(exc)}, failure.get("source_id"))
    ...
    await _append_audit(company_id, "POSTING_RETRY_SUCCEEDED", current_user.id, {"failure_id": failure_id, "journal_entry_id": result.get("id")}, result.get("id"))
...
async def reject_posting_failure(failure_id: str, body: ApprovalDecision, current_user: User = Depends(get_current_user)):
    ...
    now = datetime.now(timezone.utc).isoformat()
    await db.accounting_posting_failures.update_one({"id": failure_id, "company_id": company_id}, {"$set": {"status": "REJECTED", "rejected_by": current_user.id, "rejected_at": now, "rejection_reason": body.reason.strip()}})
    await _append_audit(company_id, "POSTING_FAILURE_REJECTED", current_user.id, {"failure_id": failure_id, "reason": body.reason.strip()}, failure.get("source_id"))
