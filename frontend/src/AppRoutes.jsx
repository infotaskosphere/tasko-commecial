...
function PlatformOwnerOnly({children}){const {user,loading}=useAuth();if(loading)return <AuthLoading/>;const isOwner=String(user?.email||'').trim().toLowerCase()==='info.taskosphere@gmail.com';if(!isOwner)return <Navigate to="/dashboard" replace/>;return children}
