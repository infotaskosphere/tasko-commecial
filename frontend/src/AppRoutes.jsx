...
const ForgotEmail = lazy(() => import('./pages/ForgotEmail.jsx'));
...
const AIWeave = lazy(() => import('./pages/AIDocumentReader.jsx'));
...
const FinixAIWorkspace = lazy(() => import('./pages/FinixAIWorkspace.jsx'));
...
function LicensedDashboardHome() { const { user, loading, isPlatformOwner, hasPermission } = useAuth(); if (loading) return <AuthLoading />; const isInternalAdmin=user?.role?.toLowerCase()==='admin'&&!user?.company_id; const canAccessTaskosphere = Boolean(isPlatformOwner) || Boolean(hasPermission?.('can_access_taskosphere')) || user?.permissions?.can_access_taskosphere===true; if(isInternalAdmin||canAccessTaskosphere)return <Dashboard/>; const destination=isCommercialTenant(user)?firstAccessiblePath(user):(['/finix-dashboard','/compliance-dashboard','/records-dashboard','/client-proposals-dashboard','/people-matrix'].find((path)=>{ const flag={ '/finix-dashboard':'can_access_finix','/compliance-dashboard':'can_access_compliance','/records-dashboard':'can_access_records','/client-proposals-dashboard':'can_access_proposals','/people-matrix':'can_access_people_matrix' }[path]; return user?.permissions?.[flag]===true; })||null); if(!destination||destination==='/login')return <NoModuleAccess/>; return <Navigate to={destination} replace/>; }
...
<Route path="/admin-dashboard" element={<AdminOnly><AdminDashboard/></AdminOnly>}/><Route path="/permission-matrix" element={<AdminOnly><PermissionMatrix/></AdminOnly>}/><Route path="/master-data" element={<AdminOnly><PageGuard module="admin" page="can_view_master_data"><MasterData/></PageGuard></AdminOnly>}/><Route path="/roles" element={<AdminOnly><PageGuard module="admin" page="can_view_roles"><Roles/></PageGuard></AdminOnly>}/><Route path="/contact-details" element={<AdminOnly><ContactDetails/></AdminOnly>}/><Route path="/master-console" element={<PlatformOwnerOnly><MasterConsole/></PlatformOwnerOnly>}/><Route path="/master-console/website" element={<PlatformOwnerOnly><WebsiteBuilder/></PlatformOwnerOnly>}/><Route path="/website-studio" element={<PlatformOwnerOnly><WebsiteBuilder/></PlatformOwnerOnly>}/><Route path="/website-builder" element={<PlatformOwnerOnly><WebsiteBuilder/></PlatformOwnerOnly>}/><Route path="/settings" element={<Navigate to="/settings/general" replace/>}/><Route path="/settings/general" element={<GeneralSettings/>}/><Route path="/settings/backup" element={<BackupPermission><BackupRestore/></BackupPermission>}/><Route path="/settings/email" element={<EmailSettings/>}/><Route path="/settings/whatsapp" element={<WhatsAppSettings/>}/><Route path="/settings/automation" element={<Navigate to="/settings/whatsapp" replace/>}/><Route path="/automation/approvals" element={<AdminOnly><PendingApprovals/></AdminOnly>}/>
