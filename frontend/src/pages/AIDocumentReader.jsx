...
import {
  ...
  deleteAccount, getModels, getRoutingConfig, updateRoutingConfig,
...
export default function AIDocumentReader() {
  ...
  const [files,setFiles]=useState([]), [connectOpen,setConnectOpen]=useState(false), [connectProvider,setConnectProvider]=useState("openai");
  const [connectName,setConnectName]=useState(""), [identity,setIdentity]=useState(""), [secret,setSecret]=useState(""), [baseUrl,setBaseUrl]=useState("");
  const [connecting,setConnecting]=useState(false), [modelSearch,setModelSearch]=useState(""), [auditSearch,setAuditSearch]=useState("");
  ...
  return <div className="flex h-[calc(100vh-64px)] min-h-[650px] w-full min-w-0 overflow-hidden bg-white text-slate-900">
    ...
    {settings&&<div className="fixed inset-0 z-50 flex bg-slate-900/30"><div className="ml-auto flex h-full w-full max-w-5xl flex-col bg-white shadow-2xl"><div className="flex items-center justify-between border-b px-5 py-4"><div><b>AIWeave Settings</b><div className="text-xs text-slate-500">Providers, accounts, models, routing, audit and documents</div></div><button onClick={()=>setSettings(false)}><X/></button></div><div className="flex min-h-0 flex-1"><nav className="hidden w-48 shrink-0 border-r bg-slate-50 p-3 sm:block">{[["providers","Providers"],["accounts","Accounts"],["models","Models"],["routing","Routing"],["audit","Audit"],["documents","Documents"]].map(x=><button key={x[0]} onClick={()=>setTab(x[0])} className={`mb-1 w-full rounded-lg px-3 py-2 text-left text-sm ${tab===x[0]?"bg-white font-semibold shadow-sm":""}`}>{x[1]}</button>)}</nav><div className="min-w-0 flex-1 overflow-y-auto p-4 sm:p-6">
      {tab==="providers"&&<div><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold">AI Providers</h2><button onClick={()=>setConnectOpen(true)} className="rounded-lg bg-[#0D3B66] px-3 py-2 text-xs font-semibold text-white"><Plus size={14} className="mr-1 inline"/>Connect account</button></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{PROVIDERS.map(p=>{const x=providers.find(y=>y.id===p.id);return <div key={p.id} className="rounded-xl border p-4"><div className="flex justify-between"><b className="text-sm">{p.name}</b><span className={x?.healthyCount?"text-emerald-600":"text-slate-300"}>●</span></div><div className="mt-1 text-[11px] text-slate-500">{x?.connectedCount||0} connected · {x?.healthyCount||0} healthy</div><button onClick={()=>{setConnectProvider(p.id);setConnectOpen(true)}} className="mt-3 w-full rounded-lg border px-3 py-2 text-xs">Connect</button></div>})}</div></div>}
      {tab==="accounts"&&<div><h2 className="mb-4 text-lg font-semibold">Accounts</h2><div className="space-y-2">{accounts.map(a=><div key={a.id} className="rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><b className="text-sm">{a.name}</b><div className="text-xs text-slate-500">{pname(a.provider)} · {a.maskedIdentity||"protected credential"}</div></div><div className="flex gap-2"><Bubble tone={a.health==="HEALTHY"?"bg-emerald-50 text-emerald-700":"bg-amber-50 text-amber-700"}>{a.health||"UNKNOWN"}</Bubble><button onClick={()=>test(a)} className="rounded-lg border px-2 py-1 text-xs">Test</button><button onClick={()=>toggle(a)} className="rounded-lg border px-2 py-1 text-xs">{a.enabled?"Disable":"Enable"}</button><button onClick={()=>remove(a)} className="rounded-lg border border-red-200 px-2 py-1 text-red-600"><Trash2 size={13}/></button></div></div></div>)}{!accounts.length&&<div className="rounded-xl border border-dashed p-8 text-center text-sm text-slate-500">No connected AI accounts.</div>}</div></div>}
    ...
    {connectOpen&&<div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 p-4"><form onSubmit={connect} className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl"><div className="flex justify-between"><div><b>Connect AI account</b><p className="text-xs text-slate-500">Credential is encrypted server-side.</p></div><button type="button" onClick={()=>setConnectOpen(false)}><X size={18}/></button></div><div className="mt-4 grid gap-3"><select value={connectProvider} onChange={e=>setConnectProvider(e.target.value)} className="rounded-lg border px-3 py-2 text-sm">{PROVIDERS.filter(p=>p.id!=="replit"&&p.id!=="lovable"&&p.id!=="mulerun").map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select><input value={connectName} onChange={e=>setConnectName(e.target.value)} placeholder="Account name" className="rounded-lg border px-3 py-2 text-sm"/><input value={identity} onChange={e=>setIdentity(e.target.value)} placeholder="Email / identity label" className="rounded-lg border px-3 py-2 text-sm"/><input value={secret} onChange={e=>setSecret(e.target.value)} type="password" placeholder={connectProvider==="ollama"?"Leave blank for local Ollama":"API key / credential"} className="rounded-lg border px-3 py-2 text-sm"/>{connectProvider==="ollama"&&<input value={baseUrl} onChange={e=>setBaseUrl(e.target.value)} placeholder="http://127.0.0.1:11434" className="rounded-lg border px-3 py-2 text-sm"/>}</div><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={()=>setConnectOpen(false)} className="rounded-lg border px-4 py-2 text-sm">Cancel</button><button disabled={connecting} className="rounded-lg bg-[#0D3B66] px-4 py-2 text-sm font-semibold text-white">{connecting?"Connecting...":"Connect account"}</button></div></form></div>}
  </div>;
}
