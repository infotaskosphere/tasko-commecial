        ...
        <section className="flex min-w-0 flex-1 flex-col">
          <div className="mx-auto flex w-full max-w-5xl min-h-0 flex-1 flex-col px-3 sm:px-6">
            ...
            <div className="pb-4 pt-2">{files.length>0&&<div className="mb-2 flex flex-wrap gap-2">{files.map((f,i)=><Bubble key={i} tone="bg-slate-100 text-slate-600"><FileText size={12} className="mr-1"/>{f.name}<button className="ml-1" onClick={()=>setFiles(x=>x.filter((_,n)=>n!==i))}><X size={11}/></button></Bubble>)}</div>}
              <div className="rounded-2xl border border-slate-300 bg-white p-2.5 shadow-sm transition focus-within:border-[#1F6FB2] focus-within:ring-2 focus-within:ring-[#1F6FB2]/10"><textarea value={prompt} onChange={e=>setPrompt(e.target.value)} onKeyDown={keyDown} disabled={running} rows={2} className="max-h-48 min-h-[64px] w-full resize-none border-0 bg-transparent px-3 py-2 text-sm outline-none" placeholder="Ask AIWeave anything..."/><div className="flex flex-wrap items-center justify-between gap-2 px-2 pb-1"><div className="flex items-center gap-1"><input ref={fileRef} type="file" multiple accept={ACCEPTED} className="hidden" onChange={e=>attach(e.target.files)}/><button onClick={()=>fileRef.current?.click()} className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50"><Paperclip size={15}/>Attach</button><button onClick={()=>fileRef.current?.click()} className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50"><Upload size={15}/>Upload</button><select value={capability} onChange={e=>setCapability(e.target.value)} className="max-w-[150px] rounded-lg bg-slate-50 px-2 py-1.5 text-[11px]">{CAPABILITIES.filter(x=>["chat","reasoning","coding","debugging","document_analysis","vision","structured_output"].includes(x.id)).map(x=><option key={x.id} value={x.id}>{x.label}</option>)}</select></div><div className="flex items-center gap-3"><span className="hidden text-[10px] text-slate-400 lg:block">Press Enter to send · Shift+Enter for new line</span><button onClick={send} disabled={!prompt.trim()||running} className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#0D3B66] text-white shadow-sm disabled:opacity-40">{running?<Square size={14}/>:<ArrowUp size={16}/>}</button></div></div></div><div className="mt-2 flex justify-between px-2 text-[10px] text-slate-400"><span>{status||"AIWeave can automatically switch models when a provider becomes unavailable."}</span><span className="hidden sm:block">Enter to send · Shift+Enter for new line</span></div>
            </div>
          </div>
        </section>
