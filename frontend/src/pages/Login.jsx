    ...
    <div className="min-h-screen overflow-x-hidden bg-[#f6f9fc] text-slate-900">
      <div className="mx-auto flex min-h-screen w-full max-w-[1800px] flex-col">
        <header className="flex h-[82px] shrink-0 items-center justify-between border-b border-slate-200/80 bg-white px-6 sm:px-10 xl:px-14">
          <Link to="/" className="inline-flex items-center">
            <img src={logo} alt={siteName} className="h-12 w-auto max-w-[230px] object-contain" />
          </Link>
          <nav className="flex items-center gap-5">
            <Link to="/" className="hidden items-center gap-2 text-sm font-semibold text-slate-500 hover:text-[#102f62] sm:inline-flex">
              <Globe2 size={15} /> Explore ONENEXA
            </Link>
            <Link to="/" className="border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-[#102f62] transition hover:border-slate-300 hover:bg-slate-50">
              Request a demo
            </Link>
          </nav>
        </header>
        ...
        <div className="grid flex-1 lg:grid-cols-[minmax(0,1fr)_410px]">
          <main className="relative flex min-w-0 overflow-hidden px-6 py-10 sm:px-10 lg:px-14 xl:px-20">
            ...
            <div className="relative mx-auto flex w-full max-w-[1080px] flex-col justify-center">
              <div className="max-w-[940px]">
                <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[.24em] text-[#1769ff]">
                  <span className="h-px w-8 bg-[#1769ff]" />
                  {config?.hero_badge || "The modern business operating system"}
                </div>

                <h1 className="mt-6 max-w-[920px] text-5xl font-black leading-[.96] tracking-[-.06em] text-[#102f62] sm:text-6xl xl:text-[5.25rem]">
                  {config?.hero_title || <>Everything your business needs.<br /><span className="text-[#1769ff]">Nothing scattered.</span></>}
                </h1>

                <p className="mt-7 max-w-[760px] text-base leading-7 text-slate-600 sm:text-lg">
                  {config?.hero_subtitle || "A connected operating layer for work, finance, people, compliance and growth — designed to keep the business moving from one place."}
                </p>
              </div>
              ...
              <section className="mt-12 border-y border-slate-200/80 py-7">
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
                  {productSuite.map((product) => (
                    <div key={product.name} className="group min-w-0 border border-slate-200 bg-white/90 px-4 py-4 transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_14px_32px_rgba(15,23,42,.07)]">
                      <div className="flex h-12 items-center">
                        <img src={product.logo} alt={product.name} className="max-h-10 max-w-[132px] object-contain" loading="lazy" />
                      </div>
                      <div className="mt-3 border-t border-slate-100 pt-3">
                        <p className="text-xs font-extrabold text-[#173c70]">{product.name}</p>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-x-8 gap-y-2 text-[10px] text-slate-400">
                ...
                <span className="sm:ml-auto">© 2026 ONENEXA</span>
              </div>
            </div>
          </main>
          ...
          <aside className="relative flex min-w-0 flex-col border-t border-slate-200 bg-white px-6 py-9 sm:px-10 lg:border-l lg:border-t-0 lg:px-10 xl:px-12">
            <div className="my-auto w-full max-w-[350px] mx-auto">
              <div className="mb-8">
                <div className="mb-5 flex h-10 w-10 items-center justify-center border border-blue-100 bg-blue-50 text-[#1769ff]"><LockKeyhole size={19} /></div>
                <h2 className="text-3xl font-black tracking-[-.045em] text-[#102f62]">{mode === "signin" ? "Welcome back." : "Activate your workspace."}</h2>
                <p className="mt-2 text-sm leading-6 text-slate-500">{mode === "signin" ? "Sign in to continue to your ONENEXA workspace." : "Verify your license and create the first administrator account."}</p>
              </div>
