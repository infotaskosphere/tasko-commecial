...
export default function PlatformUserManager({ companies: suppliedCompanies = [] }) {
  const [directoryCompanies, setDirectoryCompanies] = useState([]);
  const companies = suppliedCompanies.length ? suppliedCompanies : directoryCompanies;
  ...
  useEffect(() => {
    if (suppliedCompanies.length) return;
    api.get('/commercial-master-data/company-directory')
      .then((response) => {
        const flattened = (response.data?.customers || []).flatMap((customer) => customer.legal_companies || []);
        setDirectoryCompanies(flattened);
      })
      .catch(() => setDirectoryCompanies([]));
  }, [suppliedCompanies]);
  ...
  return <section className="master-data-users-card">
    <header className="master-data-section-header">
      <div className="master-data-section-heading"><span className="master-data-icon master-data-icon-blue"><Users2 /></span><div><div className="flex items-center gap-2 flex-wrap"><h2>User Details &amp; Access</h2><Badge className="master-data-badge">{users.length}</Badge></div><p>Platform-level user administration. Select a legal company, then add and maintain its users without exposing any other customer operational data.</p></div></div>
